...
import ChildCareAccordion from './ChildCareAccordion';
...
const ChildCareServicesSection = () => {
  const [openId, setOpenId] = useState('full-time-nannies');

  const scrollToService = (id) => {
    window.setTimeout(() => {
      document
        .getElementById(`child-care-${id}`)
      ...
      <div className="lg:w-[min(120%,calc(100vw-2rem))] lg:relative lg:left-1/2 lg:-translate-x-1/2">
        <div className="overflow-x-auto pb-2 -mx-4 px-4 sm:-mx-6 sm:px-6 lg:mx-0 lg:px-0 lg:overflow-visible">
          <div className="flex gap-3 sm:gap-4 min-w-max lg:min-w-0 lg:grid lg:grid-cols-5">
            {CHILD_CARE_SERVICES.map((service, index) => (
              <div
                key={service.id}
...
export default ChildCareServicesSection;
