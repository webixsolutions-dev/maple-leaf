...
const FAQ = () => {
  // All FAQ items are OPEN by default
  const [openItems, setOpenItems] = useState(() => faqs.map(() => true));

  const allOpen = openItems.every(Boolean);

  // Toggle a single FAQ card
  const toggleItem = (i) => {
    setOpenItems((prev) => prev.map((v, idx) => (idx === i ? !v : v)));
  };

  // Toggle ALL FAQ cards at once (master arrow)
  const toggleAll = () => {
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        >
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {/* Master toggle: expand / collapse ALL FAQs */}
            <button
              onClick={toggleAll}
              aria-expanded={allOpen}
              className="ml-auto inline-flex items-center gap-2 text-[#c72a7a] font-semibold text-sm font-montserrat border border-pink-200 rounded-xl px-4 py-2 hover:bg-pink-50 transition-all"
            >
